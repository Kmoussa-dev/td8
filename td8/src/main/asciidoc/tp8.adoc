...
vous pouvez utiliser un squelette de test abstrait dao.AbstractDaoTest.java :
...
package dao;
    ...
    @Transactional
    public abstract class dao.AbstractDaoTest extends AbstractTransactionalJUnit4SpringContextTests {
        @Autowired
        DataSource dataSrc;
        @PersistenceContext
